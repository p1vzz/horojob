...
export const DailyAstroStatus = () => {
  ...
  useEffect(() => {
    ...
    <View className="px-5 py-2">
      <LinearGradient
        colors={['rgba(201,168,76,0.06)', 'rgba(90,58,204,0.06)']}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 1 }}
      ...
      >
        ...
        >
          ...
          />
          {Platform.OS === 'web' ? (
            <Svg width={240} height={240} style={{ position: 'absolute', opacity: 0.18 }}>
              <Defs>
                <SvgRadialGradient id="astroRingsWeb" cx="50%" cy="50%" r="50%">
                  <Stop offset="0%" stopColor="rgba(201,168,76,0.65)" />
                  <Stop offset="26%" stopColor="rgba(201,168,76,0.22)" />
                  <Stop offset="36%" stopColor="rgba(201,168,76,0.00)" />
                  <Stop offset="48%" stopColor="rgba(201,168,76,0.18)" />
                  <Stop offset="58%" stopColor="rgba(201,168,76,0.00)" />
                  <Stop offset="70%" stopColor="rgba(201,168,76,0.14)" />
                  <Stop offset="80%" stopColor="rgba(201,168,76,0.00)" />
              ...
              <Canvas style={{ position: 'absolute', width: 240, height: 240, top: -60, right: -60 }}>
                  {/* Group позволяет нам сжать круг в эллипс, как в CSS rx/ry */}
                  <Group transform={[{ scaleX: 1.5 }, { scaleY: 0.8 }, { rotate: Math.PI / 4 }]}>
                      <Rect x={0} y={0} width={100} height={100}>
                          <RadialGradient
                              c={vec(180, 60)} // Смещаем центр в верхний правый угол
                              r={150}
                              colors={[
                                  'rgba(201,168,76,0.35)', // Мягкое ядро
                                  'rgba(201,168,76,0.10)', // Плавный переход
                                  'transparent'            // Полное исчезновение
                              ]}
                              positions={[0, 0.4, 1]}   // Растягиваем переход
                          />
                      </Rect>
                      <Blur blur={20} />
                  </Group>
        ...
        <View className="flex-row items-center mb-3">
          ...
          <Text className="text-[10px] ml-auto" style={{ color: 'rgba(212,212,224,0.4)' }}>
        ...
        <View className="mb-5">
          ...
          <View className="flex-row items-center min-h-[44px]">
            <Text className="text-[14px] leading-[22px]" style={{ color: 'rgba(212,212,224,0.75)' }}>
              {typed}
            </Text>
          </View>
        </View>
          ...
          {[
            ...
            <View key={stat.label} className="flex-1">
              <View className="flex-row justify-between items-center mb-1.5">
                <Text className="text-[10px] uppercase tracking-[1px] font-semibold" style={{ color: 'rgba(212,212,224,0.45)' }}>
                  {stat.label}
                </Text>
                <Text className="text-[10px] font-bold" style={{ color: stat.color }}>{stat.value}%</Text>
              </View>
              <View className="h-1 rounded-full overflow-hidden" style={{ backgroundColor: 'rgba(255,255,255,0.06)' }}>
                <View 
